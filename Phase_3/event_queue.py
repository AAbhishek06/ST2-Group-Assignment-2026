...
# Event structure
class Event:
    def __init__(self, name, priority, duration, timestamp):
        self.name = name
    ...
    def __lt__(self, other):
        ...
        # If still same, earlier timestamp first
...
class EventQueueVisualiser:
    ...
    def create_buttons(self):
        ...
        total_width = len(labels) * button_width + (
            len(labels) - 1
        ) * spacing
    ...
    def get_active_text(self):
        ...
        if self.active_input == "priority":
            return self.input_priority

        if self.active_input == "duration":
    ...
    def add_event(self):
        ...
        if (
            not self.input_name.strip()
            or not self.input_priority.isdigit()
            or not self.input_duration.isdigit()
        ):
            self.status = "Enter valid name, priority, and time"
            return

        event = Event(
            self.input_name.strip(),
            int(self.input_priority),
            int(self.input_duration),
            time.time()
        )
    ...
    def add_test_event(self):
        ...
        if self.test_index < len(self.test_items):
            ...
            name, priority, duration = (
                self.test_items[self.test_index]
            )
    ...
    def draw_processed(self, screen, fonts):
        ...
        for event in self.processed_events[-6:]:
            ...
            ui.draw_node_rect(
                screen,
                self.processed_panel.x + 40,
                y,
                f"{event.name} P:{event.priority} T:{event.duration}",
                fonts["small"],
                highlight=False
            )
...
def run_event_queue(screen, clock):
    ...
    while running:
        ...
        back = ui.draw_back_button(
            screen,
            fonts["small"],
            mouse
        )

        test = ui.draw_test_button(
            screen,
            fonts["small"],
            mouse
        )
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    vis.set_active_text(
                        vis.get_active_text()[:-1]
                    )

                elif event.unicode.isprintable():

                    vis.set_active_text(
                        vis.get_active_text() + event.unicode
                    )
                ...
                elif vis.time_box.collidepoint(event.pos):
                    vis.active_input = "duration"
